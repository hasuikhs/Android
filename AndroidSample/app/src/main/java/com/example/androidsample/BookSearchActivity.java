...
class SearchTitleRunnable implements Runnable{
    ...
    @Override
    public void run() {
        // keyword를 이용해서 web program에 접속 한 후 결과를 받아옴
        // 결과로 받아온 JSON문자열을 이용해서 Listview에 출력해야 함
        // 그런데 여기서는 Listview를 제어 불가능
        // Handler를 이용해서 UI Thread에 Listview에 사용할 데이터를 전달
        String url = "http://70.12.115.53:8080/bookSearch/searchTitle?USER_KEYWORD=" + keyword;

        // network code는 예외처리 필요
        try {
...
public class BookSearchActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final Handler handler = new Handler(){
            // handler에게 message가 전달되면 아래의 method가 callback
            @Override
        ...
        // UI Thread(Activity Thread, Main Thread)에서는 Network 연결 불가능

        searchBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                // 사용자가 입력한 keyword를 가지고 Thread를 파생
                SearchTitleRunnable runnable = new SearchTitleRunnable(handler,
                                                            keywordEt.getText().toString());
